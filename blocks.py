...
class Maskedtemporal(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # output projection + skip connection
        out = x + self.proj_drop(self.proj(out)) * mask.to(out.dtype)
        return out, mask
...
class CaPBlock(nn.Module):
    def __init__(
        ...
        up_size = round(kernel_size * k)
        up_size = up_size + 1 if up_size % 2 == 0 else up_size

        self.g_conv = nn.Conv1d(n_embd, n_embd, 1, stride=1, padding=0, groups=n_embd)
        self.key_conv = MaskedConv1D(n_embd, n_embd, up_size, stride=n_ds_stride, padding=up_size // 2, groups=n_embd)
        self.value_conv = MaskedConv1D(n_embd, n_embd, up_size, stride=n_ds_stride, padding=up_size // 2, groups=n_embd)
        ...
        if n_ds_stride > 1:
            ...
            # self.pool_skip = nn.Sequential(nn.AvgPool1d(n_ds_stride, stride=n_ds_stride, padding=0),
        ...
        #     nn.Conv1d(n_embd+1, n_embd//2, 3, 1, 1),
        #     act_layer(),
        #     nn.Conv1d(n_embd//2, 1, 1)
        # )
    ...
    def forward(self, x, mask, g_fea):

        # X shape: B, C, T    g_fea shape: B C 1

        x = self.ln1(x)

        query = self.g_conv(g_fea)
        k, out_mask = self.key_conv(x, mask)
        v, _ = self.value_conv(x, mask)
        ...
        # else:
        #     query = g_fea
        #     k= nn.AvgPool1d(self.stride, stride=self.stride, padding=0)(x)
        #     v= nn.AvgPool1d(self.stride, stride=self.stride, padding=0)(x)

        # att = torch.relu(F.cosine_similarity(query, k, dim=1).unsqueeze(1) * out_mask.to(k.dtype))
        att = torch.relu((query.transpose(-2, -1) @ k) * -1) * out_mask.to(k.dtype)
        out = self.pool_skip(x) * out_mask.to(k.dtype) + self.drop_path_out(att * v)

        out = out + self.drop_path_mlp(self.mlp(self.ln2(out)) * out_mask.to(k.dtype))

        if self.is_affine:
            g_att = F.normalize(out, dim=1).transpose(-2, -1) @ F.normalize(g_fea, dim=1)  # B T 1
            g_att = g_att.masked_fill(torch.logical_not(out_mask.transpose(-2, -1)), float('-inf'))  # B T 1
            g_att = F.softmax(g_att, dim=-2)
            g_fea = self.gamma * g_fea + (1 - self.gamma) * ((out * out_mask.to(x.dtype)) @ g_att)  # B C 1

            # frame_score = F.cosine_similarity(g_fea, out).unsqueeze(1) * out_mask.to(out.dtype)  # B 1 T
            # frame_score = self.frame_mlp(torch.cat([out, frame_score], dim=1)) * out_mask.to(out.dtype)  # B 1 T
            # g_att = frame_score.masked_fill(torch.logical_not(out_mask), float('-inf'))
            # g_att = F.softmax(g_att, dim=-1)
            # g_fea = self.gamma * g_fea + (1 - self.gamma) * ((out * out_mask.to(x.dtype)) @ g_att.transpose(-2, -1))  # B C 1

        else:
            g_fea = g_fea

        return out, out_mask, g_fea
...
if __name__ == '__main__':
    ...
    op = MaskedMHCA(16, 2)
    a = torch.randn(2, 16, 10)
    b = torch.Tensor([[[True, False, False, True, False, True, True, True, True, False]],
