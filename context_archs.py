...
class Contextformer(nn.Module):
    ...
    def __init__(
        ...
        backbone_arch=(2, 1, 5),         # a tuple defines #layers in embed / stem / branch    # (2, 1, 5)
        ...
        n_cap_conv_size=[3, 3, 3, 3, 3, 3],      # win size for cap      [3, 3, 3, 3, 3, 3]
        ...
        regression_range=[(0, 4), (4, 8), (8, 16), (16, 32), (32, 64), (64, 10000)],      # regression range on each level of FPN  [(0, 4), (4, 8), (8, 16), (16, 32), (32, 64), (64, 10000)]
        ...
        self.train_center_sample = "radius"  # radius
        assert self.train_center_sample in ['radius', 'none']
        self.train_center_sample_radius = 1.5
        self.train_loss_weight = 2.0
    ...
    def forward(self, video_list):
        ...
        if self.backbone_type == 'convTransformer':
            ...
            # feats, masks, gfeats, fscroes = self.backbone(batched_inputs, batched_masks)
        ...
        if self.fpn_type == 'identity':
            ...
            # fpn_feats, fpn_masks, fpn_gfeats, fpn_scores = self.neck(feats, masks, gfeats, fscroes)
        ...
        if self.training:
            ...
            # sim_scores = self.sim_score(video_list, fpn_feats, fpn_masks)
            return results
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        # print(f"Type of first element: {type(video_list)}")

        feats = [x['feats'] for x in video_list]
        # feats = [video_list['feats']]
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label, gt_frame_label):
        ...
        for idx in range(self.backbone_arch[2]-2):
            tmp_frame_label = nn.AvgPool1d(kernel_size=2**(idx+1), stride=2**(idx+1))(gt_frame_label[None][None])
            tmp_frame_label = torch.where(tmp_frame_label > 0.35,
                                          torch.full_like(tmp_frame_label, 1.),
                                          torch.full_like(tmp_frame_label, 0.))
            frame_targets += (tmp_frame_label.squeeze(), )
    ...
    def losses2(
        ...
        # center_loss = self.center_loss(fpn_feats, fpn_gfeats, gt_fpn_frame)
        ...
        return {'cls_loss'   : cls_loss,
                ...
                'info_loss'  : info_loss*0.5,
                # 'frame_loss' : frame_loss,
                'final_loss' : final_loss}

    @torch.no_grad()
    def sim_score(
            self, video_list, fpn_feats, fpn_masks
    ):
        batch_sim_scores=[]
        for idx in range(len(video_list)):
            fpn_feats_per_vid = [x[idx] for x in fpn_feats]
            fpn_masks_per_vid = [x[idx] for x in fpn_masks]

            sim_scores = []
            for feats_i, masks_i in zip(fpn_feats_per_vid, fpn_masks_per_vid):
                indices = torch.where(masks_i)[0]
                valid_feat = feats_i[:, indices]   # C, valid_T
                mean_feat = valid_feat.mean(-1, keepdim=True)  # C 1
                avg_cosine_similarity = F.cosine_similarity(valid_feat, mean_feat, dim=0).mean()
                sim_scores.append(avg_cosine_similarity)

            sim_scores = torch.stack(sim_scores)  # 6
            batch_sim_scores.append(sim_scores)

        batch_sim_scores = torch.stack(batch_sim_scores, dim=1)

        return batch_sim_scores
